...
describe('spreadsheet style geometry', () => {
  ...
  test('expands rows and headers to fit their typography and vertical padding', () => {
    ...
    expect(spreadsheetViewportHeight('fit', 2, {}, geometry)).toBe(186);
    // The add-row strip is one configured row tall.
    expect(
      spreadsheetViewportHeight('fit', 2, { addRow: true }, geometry)
    ).toBe(252);
