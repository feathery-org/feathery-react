...
export function spreadsheetViewportHeight(
  ...
  const content =
    ...
    (rowCount + (chrome.addRow ? 1 : 0)) * geometry.rowHeight;
...
export const columnResizerStyle = {
  ...
  '&:hover::after': { backgroundColor: v('accent_color', colors.accent) }
} as const;

export const columnResizerActiveStyle = {
  '&::after': { backgroundColor: v('accent_color', colors.accent) }
} as const;
...
export const choiceOptionStyle = (highlighted: boolean, empty: boolean) =>
  ({
    ...
    color: empty
      ? colors.gray500
      : v('editor_font_color', v('font_color', colors.gray900)),
    backgroundColor: highlighted
      ? v('selected_background_color', colors.accentSoft)
      : 'transparent',
