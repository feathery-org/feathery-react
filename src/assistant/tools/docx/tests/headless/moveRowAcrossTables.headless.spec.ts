...
const move = (target: string, source: string, row: number, group: string) => [
  ...
  { op: 'set_cell_text', group, anchor: `${target};${row + 1};0;0`, text: 'Buildings' },
  { op: 'set_cell_text', group, anchor: `${target};${row + 1};1;0`, text: '12' },
  { op: 'set_cell_text', group, anchor: `${target};${row + 1};2;0`, text: '$640.50' },
...
describe('a row moves between tables by primitives, before and after a split', () => {
  ...
  it('before any split: Buildings moves from the Property table into the Liability table', async () => {
    ...
    const property = await session.call<string>('tableAnchor', 'property_premium');
    const liability = await session.call<string>('tableAnchor', 'liability_premium');
    const moved = await session.call<any>('applyEdits', move(liability, property, 1, 'g01'), 'across-before');
    ...
    const formulas = await session.call<Record<string, string>>('formulaValues');
  ...
  it('after a split: a row moves into the copy, then more rows anchor on the moved row and on the subtotal row', async () => {
    ...
    expect((await session.call<any>('applyEdits', [{ op: 'split_table', group: 'g01', anchor: source, rows: [3] }], 'across-split')).outcomes).toEqual(['ok']);
    await session.call('resolveGroups', true);
    const copy = await session.call<string>('tableAnchor', 'property_premium_copy');
    // The copy holds Stock (row 1) and its subtotal (row 2); Buildings joins below Stock.
    const moved = await session.call<any>('applyEdits', move(copy, source, 1, 'g02'), 'across-move');
    expect(moved.outcomes).toEqual(['ok', 'ok', 'ok', 'ok', 'ok']);
    await session.call('resolveGroups', true);
    ...
    const onMoved = await session.call<any>('applyEdits', [
      { op: 'insert_row', group: 'g03', anchor: `${copy};2;0;0` },
      { op: 'set_cell_text', group: 'g03', anchor: `${copy};3;0;0`, text: 'Signage' },
      { op: 'set_cell_text', group: 'g03', anchor: `${copy};3;1;0`, text: '3', literal: true },
      { op: 'set_cell_text', group: 'g03', anchor: `${copy};3;2;0`, text: '$210.00', literal: true }
    ], 'across-on-moved');
    ...
    const afterTotal = await session.call<any>('applyEdits', [
      { op: 'insert_row', group: 'g04', anchor: `${copy};4;0;0` },
      { op: 'set_cell_text', group: 'g04', anchor: `${copy};5;0;0`, text: 'Fixtures' },
      { op: 'set_cell_text', group: 'g04', anchor: `${copy};5;1;0`, text: '2', literal: true },
      { op: 'set_cell_text', group: 'g04', anchor: `${copy};5;2;0`, text: '$100.00', literal: true }
    ], 'across-after-total');
    expect(afterTotal.outcomes).toEqual(['ok', 'ok', 'ok', 'ok']);
    await session.call('resolveGroups', true);
    formulas = await session.call<Record<string, string>>('formulaValues');
    expect(formulas.property_premium_copy_subtotal).toBe('$12,842.00');
    expect(await session.call<string[]>('tableRowTexts', 'property_premium_copy')).toEqual(
      expect.arrayContaining([expect.stringContaining('Signage'), expect.stringContaining('Fixtures')])
    );
  }, 120000);
});
