...
const api = {
  ...
  /** The hand-composed split of specific rows using only public primitives. */
  composedSplitRows(
    tableId: string,
    rows: number[]
  ): { outcomes: string[]; messages: string[]; warnings: string[] } {
    const tableAnchor = tableBlockAnchor(tableId);
    const table = tableBlockOf(tableId);
    const itemRows = table.rows
      .map((_row: any, index: number) => index)
      .filter((index: number) => index > 0 && index < table.rows.length - 1);
    const staying = itemRows.filter((row: number) => !rows.includes(row));
    const result: any = applyDocumentEdits(live() as unknown as LiveEditor, {
      edits: [
        {
          op: 'duplicate_table',
  ...
  /** Preferred widths from the live laid-out cells in the widest table row. */
