// Completeness and honesty contracts for the reads the formula op depends on:
...
//   - a short/merged row reported as a missing cell rather than skipped;
//   - capped section/full reads carrying the same contract.
// The engine-computed write itself is tested in formulaCells.spec.ts.
...
describe('table_column reads', () => {
  ...
  it('a deliberately capped read says "N of M" instead of truncating silently (the live maxEntries-60 bug)', () => {
    ...
    expect(result.truncation.message).toContain(`60 of ${DATA_ROWS + 2} rows`);
  });
